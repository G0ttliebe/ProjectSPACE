...
class Player(pygame.sprite.Sprite):
    def __init__(self, x_pos, y_pos, width, height):
        super().__init__()
        self.images = [(pygame.image.load("Ambush in space/player.PNG")), (pygame.image.load("Ambush in space/player_death.PNG"))]
        self.dead = False
        self.delete = 0.0
        self.move = 0
        self.height = height
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x_pos, y_pos, width, height):
        super().__init__()
        self.images = [(pygame.image.load("Ambush in space/enemy_1.PNG")), (pygame.image.load("Ambush in space/enemy_death.PNG"))]
        self.dead = False
        self.delete = 0.0
        self.width = width
        self.height = height
...
class PlayerBullet(pygame.sprite.Sprite):
    def __init__(self, x_pos, y_pos, width, height):
        ...
        self.image = pygame.image.load("Ambush in space/player_bullet.PNG")
        self.image = pygame.transform.scale(self.image, (width, height))
        self.rect = self.image.get_rect()
        self.rect.x, self.rect.y = x_pos, y_pos
...
class EnemyBullet(pygame.sprite.Sprite):
    def __init__(self, x_pos, y_pos, width, height):
        ...
        self.image = pygame.image.load("Ambush in space/enemy_bullet.PNG")
        self.image = pygame.transform.scale(self.image, (width, height))
        self.rect = self.image.get_rect()
        self.rect.x, self.rect.y = x_pos, y_pos
...
pygame.display.set_caption("Ambush in space!")
background = (0, 0, 0)

icon = pygame.image.load("Ambush in space/window_icon.PNG")
...
try:
    with open("Ambush in space/save_data.txt") as save_file:
...
enemy_kill = pygame.mixer.Sound("Ambush in space/explode1.wav")
player_kill = pygame.mixer.Sound("Ambush in space/explode2.wav")
shoot_sound = pygame.mixer.Sound("Ambush in space/shoot2.wav")
select_sound = pygame.mixer.Sound("Ambush in space/select1.wav")
shoot_sound.set_volume(0.2)
select_sound.set_volume(0.4)


main_font = pygame.font.Font("Ambush in space/PressStart2P.ttf", 16)


title_surf = main_font.render(f"Ambush in space!", True, (255, 255, 255))
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if play_but_rect.collidepoint(event.pos) and menu == "main":
                ...
                with open("Ambush in space/save_data.txt", "w") as save_file:
        ...
        if play_but_rect.collidepoint(pygame.mouse.get_pos()):
            play_but_surf = main_font.render(f"easter egg", True, (255, 255, 255))
            play_but_rect = play_but_surf.get_rect(center=(screen_width/2, screen_height/2 + 16))
        else:
            play_but_surf = main_font.render(f"PLAY?", True, (255, 255, 255))
            play_but_rect = play_but_surf.get_rect(center=(screen_width / 2, screen_height / 2 + 16))
        ...
        if menu_but_rect.collidepoint(pygame.mouse.get_pos()):
            menu_but_surf = main_font.render(f"-RETURN TO BASE-", True, (255, 255, 255))
            menu_but_rect = menu_but_surf.get_rect(center=(screen_width / 2, screen_height / 2 + 48))
        else:
            menu_but_surf = main_font.render(f"RETURN TO BASE", True, (255, 255, 255))
            menu_but_rect = menu_but_surf.get_rect(center=(screen_width / 2, screen_height / 2 + 48))
